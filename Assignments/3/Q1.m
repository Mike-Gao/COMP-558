...
for i=1:100
    ...
    matched_1_norm = getNorm(matched_1);
    matched_2_norm = getNorm(matched_2);
    mat_a = generateA(4, matched_1, matched_2);
    h_norm = getHNormalized(mat_a);
end
...
function H = getHNormalized(mat_in)
    [U, S, V] = svd(mat_in);
    sol = V(:,end)';
    H = reshape(sol,3,3);
end